#!/usr/bin/env python3
"""Analyze maker-taker gap evolution over time.

Computes quarterly maker and taker excess returns to assess whether
the bias changes as the platform matures or with retail influxes.
"""

import json
from pathlib import Path

import duckdb
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from scipy import stats


def main():
    base_dir = Path(__file__).parent.parent.parent
    trades_dir = base_dir / "data" / "trades"
    markets_dir = base_dir / "data" / "markets"
    fig_dir = base_dir / "research" / "fig"
    fig_dir.mkdir(parents=True, exist_ok=True)

    con = duckdb.connect()

    # Compute quarterly maker-taker returns
    df = con.execute(
        f"""
        WITH resolved_markets AS (
            SELECT ticker, result
            FROM '{markets_dir}/*.parquet'
            WHERE status = 'finalized'
              AND result IN ('yes', 'no')
        ),
        all_positions AS (
            SELECT
                'taker' AS role,
                DATE_TRUNC('quarter', t.created_time) AS quarter,
                CASE WHEN t.taker_side = 'yes' THEN t.yes_price ELSE t.no_price END AS price,
                CASE WHEN t.taker_side = m.result THEN 1.0 ELSE 0.0 END AS won,
                t.count AS contracts
            FROM '{trades_dir}/*.parquet' t
            INNER JOIN resolved_markets m ON t.ticker = m.ticker

            UNION ALL

            SELECT
                'maker' AS role,
                DATE_TRUNC('quarter', t.created_time) AS quarter,
                CASE WHEN t.taker_side = 'yes' THEN t.no_price ELSE t.yes_price END AS price,
                CASE WHEN t.taker_side != m.result THEN 1.0 ELSE 0.0 END AS won,
                t.count AS contracts
            FROM '{trades_dir}/*.parquet' t
            INNER JOIN resolved_markets m ON t.ticker = m.ticker
        )
        SELECT
            role,
            quarter,
            AVG(won - price / 100.0) AS excess_return,
            VAR_POP(won - price / 100.0) AS var_excess,
            COUNT(*) AS n_trades,
            SUM(contracts * price / 100.0) AS volume_usd
        FROM all_positions
        GROUP BY role, quarter
        HAVING COUNT(*) >= 1000
        ORDER BY quarter, role
        """
    ).df()

    # Calculate standard errors and confidence intervals
    df["se"] = np.sqrt(df["var_excess"] / df["n_trades"])
    df["ci_lower"] = df["excess_return"] - 1.96 * df["se"]
    df["ci_upper"] = df["excess_return"] + 1.96 * df["se"]

    # Pivot for easier plotting
    taker_df = df[df["role"] == "taker"].copy()
    maker_df = df[df["role"] == "maker"].copy()

    # Merge to compute gap
    merged = taker_df.merge(
        maker_df,
        on="quarter",
        suffixes=("_taker", "_maker")
    )
    merged["gap"] = (merged["excess_return_maker"] - merged["excess_return_taker"]) * 100

    # Save to CSV
    output_df = merged[["quarter", "excess_return_taker", "excess_return_maker", "gap",
                        "n_trades_taker", "volume_usd_taker"]].copy()
    output_df.columns = ["quarter", "taker_return", "maker_return", "gap_pp", "n_trades", "volume_usd"]
    output_df["taker_return"] = output_df["taker_return"] * 100
    output_df["maker_return"] = output_df["maker_return"] * 100
    output_df.to_csv(fig_dir / "maker_taker_gap_over_time.csv", index=False)

    # Figure: Time series of maker-taker gap
    fig, ax1 = plt.subplots(figsize=(12, 6))

    # Convert to pandas timestamps for formatting
    merged["quarter"] = pd.to_datetime(merged["quarter"])
    quarters = merged["quarter"].values
    x = np.arange(len(quarters))
    quarter_labels = [f"{pd.Timestamp(q).year} Q{(pd.Timestamp(q).month-1)//3+1}"
                      for q in quarters]

    # Plot returns
    ax1.plot(x, merged["excess_return_taker"] * 100, color="#e74c3c",
             linewidth=2, label="Taker Return", marker="o", markersize=4)
    ax1.plot(x, merged["excess_return_maker"] * 100, color="#2ecc71",
             linewidth=2, label="Maker Return", marker="o", markersize=4)
    ax1.fill_between(x, merged["excess_return_taker"] * 100, alpha=0.2, color="#e74c3c")
    ax1.fill_between(x, merged["excess_return_maker"] * 100, alpha=0.2, color="#2ecc71")
    ax1.axhline(y=0, color="gray", linestyle="--", linewidth=0.8)

    # Mark key events
    # 2024 Q4 = election + legal victory
    election_idx = None
    for i, q in enumerate(quarters):
        ts = pd.Timestamp(q)
        if ts.year == 2024 and ts.month == 10:
            election_idx = i
            break

    if election_idx is not None:
        ax1.axvline(x=election_idx, color="blue", linestyle=":", linewidth=1.5, alpha=0.7)
        ax1.annotate("2024 Election\n& Legal Victory", xy=(election_idx, ax1.get_ylim()[1] * 0.8),
                     fontsize=9, ha="center", color="blue")

    ax1.set_xlabel("Quarter")
    ax1.set_ylabel("Excess Return (pp)")
    ax1.set_title("Maker-Taker Gap Over Time")
    ax1.set_xticks(x)
    ax1.set_xticklabels(quarter_labels, rotation=45, ha="right")
    ax1.legend(loc="upper left")

    # Add volume on secondary axis
    ax2 = ax1.twinx()
    ax2.bar(x, merged["volume_usd_taker"] / 1e9, alpha=0.15, color="gray", width=0.8)
    ax2.set_ylabel("Volume ($B)", color="gray")
    ax2.tick_params(axis="y", labelcolor="gray")

    plt.tight_layout()
    fig.savefig(fig_dir / "maker_taker_gap_over_time.png", dpi=300, bbox_inches="tight")
    fig.savefig(fig_dir / "maker_taker_gap_over_time.pdf", bbox_inches="tight")
    plt.close(fig)

    # JSON output for paper
    json_data = {
        "type": "line",
        "title": "Maker-Taker Returns Over Time",
        "data": [
            {
                "quarter": f"{pd.Timestamp(q).year} Q{(pd.Timestamp(q).month-1)//3+1}",
                "Taker Return": round(row["excess_return_taker"] * 100, 2),
                "Maker Return": round(row["excess_return_maker"] * 100, 2),
                "Volume ($B)": round(row["volume_usd_taker"] / 1e9, 2),
            }
            for q, row in zip(merged["quarter"], merged.to_dict("records"))
        ],
        "xKey": "quarter",
        "yKeys": ["Taker Return", "Maker Return"],
        "yUnit": "percent",
    }
    with open(fig_dir / "maker_taker_gap_over_time.json", "w") as f:
        json.dump(json_data, f, indent=2)

    # Print summary
    print(f"Outputs saved to {fig_dir}")
    print("\n=== Quarterly Maker-Taker Gap ===")
    print(f"{'Quarter':<12} {'Taker':>10} {'Maker':>10} {'Gap':>10} {'Volume':>12} {'N Trades':>12}")
    print("-" * 70)
    for _, row in merged.iterrows():
        ts = pd.Timestamp(row["quarter"])
        q_str = f"{ts.year} Q{(ts.month-1)//3+1}"
        print(f"{q_str:<12} {row['excess_return_taker']*100:>+9.2f}% {row['excess_return_maker']*100:>+9.2f}% "
              f"{row['gap']:>9.2f}pp ${row['volume_usd_taker']/1e9:>10.2f}B {row['n_trades_taker']:>12,.0f}")

    # Statistical test: is there a trend?
    x_numeric = np.arange(len(merged))
    slope, intercept, r_value, p_value, std_err = stats.linregress(x_numeric, merged["gap"])
    print(f"\n=== Trend Analysis ===")
    print(f"Linear trend in gap: {slope:+.3f} pp/quarter")
    print(f"R-squared: {r_value**2:.3f}")
    print(f"P-value: {p_value:.4f}")

    # Pre vs post election comparison
    pre_election = merged[merged["quarter"] < "2024-10-01"]
    post_election = merged[merged["quarter"] >= "2024-10-01"]

    if len(pre_election) > 0 and len(post_election) > 0:
        pre_gap = pre_election["gap"].mean()
        post_gap = post_election["gap"].mean()
        print(f"\n=== Pre vs Post 2024 Election ===")
        print(f"Pre-election avg gap: {pre_gap:.2f} pp (n={len(pre_election)} quarters)")
        print(f"Post-election avg gap: {post_gap:.2f} pp (n={len(post_election)} quarters)")
        print(f"Change: {post_gap - pre_gap:+.2f} pp")


if __name__ == "__main__":
    main()
