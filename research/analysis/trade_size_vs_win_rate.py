#!/usr/bin/env python3
"""Analyze trade size (USD) vs average win rate, controlling for price."""
...
def main():
    ...
    # Compute excess win rate: actual win rate minus expected win rate (price/100)
    # This controls for the fact that higher-priced contracts naturally win more often
    # Use log-scale bins to handle wide range of trade sizes
    df = con.execute(
        f"""
        WITH trade_data AS (
            SELECT
        ...
        SELECT bin_lower AS trade_size_bin, win_rate, expected_win_rate, excess_win_rate, n_trades
        FROM binned
        ORDER BY bin_lower
        """
    ).df()

    df.to_csv(fig_dir / "trade_size_vs_win_rate.csv", index=False)
    ...
    # Line of best fit (weighted by n_trades, in log space)
    ...
    ax.set_title("Trade Size vs Excess Win Rate (controlling for price)")

    plt.tight_layout()
    fig.savefig(fig_dir / "trade_size_vs_win_rate.png", dpi=300, bbox_inches="tight")
    fig.savefig(fig_dir / "trade_size_vs_win_rate.pdf", bbox_inches="tight")
    plt.close(fig)
